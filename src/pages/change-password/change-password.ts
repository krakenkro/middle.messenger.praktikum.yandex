...
export default class ChangePassword extends Block<Record<string, unknown>> {
    ...
    protected init(): void {
        const createInputField = (title: string, name: string, pattern: string, type: string = 'text'): InputField => new InputField({
        ...
        const inputFields = {
            inputFieldPassword: createInputField('Старый пароль', 'old_password', passwordPattern, 'password'),
            inputFieldNewPassword: createInputField('Новый пароль', 'new_password', passwordPattern, 'password'),
            inputFieldPasswordRepeat: createInputField('Повторите новый пароль', 'repeat_new_password', passwordPattern, 'password'),
        };
        const button = new Button({
            text: 'Сохранить',
            onClick: (e: MouseEvent) => {
                e.preventDefault();
                const oldPasswordElement = inputFields.inputFieldPassword.getContent().querySelector('input') as HTMLInputElement;
                const newPasswordElement = inputFields.inputFieldNewPassword.getContent().querySelector('input') as HTMLInputElement;
                const repeatPasswordElement = inputFields.inputFieldPasswordRepeat.getContent().querySelector('input') as HTMLInputElement;
